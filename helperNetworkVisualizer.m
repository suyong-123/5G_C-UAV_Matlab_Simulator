classdef helperNetworkVisualizer < handle
    ...
    methods
        function obj = helperNetworkVisualizer(varargin)
            ...
            %-------(su)---------
            % obj.Axis.XLabel.FontSize = 10;
            % obj.Axis.YLabel.FontSize = 10;
            % obj.Axis.FontName = 'Times New Roman';
           
            
        

            if isempty(obj.NetworkSimulator)
                obj.NetworkSimulator = wirelessNetworkSimulator.getInstance();% 获得全局网络模拟器
            end
            drawnow;
            scheduleAction(obj.NetworkSimulator, @obj.init, [], 0);%在模拟网络时间为0的时候，初始化
        end
        ...
        function showBoundaries(obj, bsCoordinates, cellRadius, varargin)
            ...
            for i = 1:numBS
                ...
                if ismember(i, cellOfInterest)%i == cellOfInterest (Su) 如果i在cellOfInterest，也即当前gNB是感兴趣gNB的话，就用特殊颜色标记
        ...
        function addNodes(obj, nodes, varargin)
            ...
            if isempty(tagIdx)% 如果未提供'Tag'，使用节点的ID
            ...
            for idx = 1:numel(nodeTags)
                ...
                node = scatter(obj.Axis, ...
                    px, ...
                    py, ...
                    Marker=obj.Markers{nodeMarkerInfo(1)}, ...
                    LineWidth=4, ...
                    Tag=nodeTags(idx));% 使用scatter绘制节点散点图，后续只要修改scatter的x,y轴即可
                
                
                if objectType=='nrUE' %绘制UE和gNB之间的连线
                    UEnode=nodes{idx};
                    gNB = UEnode.GNBNodeID; % 获取与该UE关联的gNB的ID
                ...
                if obj.ShowDatatip  %鼠标悬停功能
                    cellIdRow = dataTipTextRow("", nodes{idx}.Name);
                    posRow = dataTipTextRow('Position[X, Y]: ',{sprintf('%.2f, %.2f',px,py)});
                ...
                if newType
                    ...
                    obj.Legend{numItems,2} = objectType;% 图例
                end
    ...
    methods(Hidden)
        function init(obj,varargin)
            ...
            if isempty(obj.SampleRate) %SampleRate定义了可视化更新频率
                obj.SampleRate = obj.NumVisualizationUpdate/obj.NetworkSimulator.EndTime;
            end
            scheduleAction(obj.NetworkSimulator, @obj.visualizer, [], 1/obj.SampleRate, 1/obj.SampleRate); %周期性调用visualizer 1/100 即0.01s更新一次画面
        ...
        function visualizer(obj, varargin)
            ...
            delete(findobj(obj.Axis.Children, 'Tag', 'ConnectionLine'));% 把之前连接线段删掉(Su) Tag为connectionline表明是UE和gNB相连的线
            ...
            for idx=1:numTags
                ...
                nodeDetails = findobj(obj.Axis.Children,'Tag',obj.Tags{idx});% 更新节点位置
                ...
                if obj.ShowDatatip
                    nodeDetails.DataTipTemplate.DataTipRows(2).Value = {sprintf('%.2f, %.2f',position(1),position(2))};
                end
                
                

                if isa(obj.Nodes{idx}, 'nrUE')
                    % Draw connection line to its gNB
